...
describe("CarbonForm", () => {
  it("has no accessibility violations", async () => {
    const { container } = render(<CarbonForm onSubmit={() => {}} loading={false} />);
    expect(await axe(container)).toHaveNoViolations();
  });

  it("submits the entered values via onSubmit", async () => {
    const onSubmit = vi.fn();
    render(<CarbonForm onSubmit={onSubmit} loading={false} />);
  ...
  it("disables the submit button and marks it busy while loading", () => {
    render(<CarbonForm onSubmit={() => {}} loading={true} />);
  ...
  it("associates the household hint with its input for screen readers", () => {
    render(<CarbonForm onSubmit={() => {}} loading={false} />);
